    ...
    switch(action.type){
      case "usernameImmediately":
        ...
        if(draft.username.value && !/^([a-zA-Z0-9]+)$/.test(draft.username.value)){
          draft.username.hasErrors = true;
          draft.username.message ="Username contains only alphanumeric characters";
        }
        return;
      case "usernameAfterDelay":
        if(draft.username.value.length < 3){
          draft.username.hasErrors = true;
          draft.username.message = "username at least must be 3 characters";
        }
      ...
      case "usernameUniqueResults":
        if(action.value){
            ...
            draft.username.message = "This username has already been taken";
        }
        else{
